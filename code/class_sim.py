...
class Sim:
    ...
    # class methods
    from helpers.plotting import plot_timestep_data
    # from game import mc_estimate
    from helpers.helpers import coin_toss, get_random_float, invent_strategy, \
                        choose_strategy, choose_strategy_pair, gain_adherent, \
                        get_avg_strategy_payoff, add_strategy, \
                        lose_adherent, record_timestep_data

    from helpers.printing import print_num_s_active, print_freq_total, print_status, \
                         print_results

    from helpers.simulation import simulate_timesteps
    ...
    def reset_simulation(self):
        ...
        # currently active strategies and their frequencies        
        self.s_active = [self.s_initial]
        self.s_freqs  = [self.N]

        new_s_payoff, _, new_s_cc_rate, _, new_s_game1_rate = \
            self.game.get_stats(self.s_initial, self.s_initial)
